            ...
            if output.multi_hand_landmarks:
                for landmarks in output.multi_hand_landmarks:
                    for id , pos in enumerate(landmarks.landmark):
                        heigth,width,z = image.shape
                        pos_x,pos_y = int(width*(pos.x)),int(heigth*(pos.y))
                        if id == 8:
                            cv2.circle(image, (300, 300), 200, Blue, cv2.FILLED)
                            cv2.circle(image, (300, 300), 100, Red, cv2.FILLED)
